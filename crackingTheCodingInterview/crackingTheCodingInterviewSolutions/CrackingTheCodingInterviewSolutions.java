package crackingTheCodingInterviewSolutions;
...
class FuckOffException extends RuntimeException{
  private static final long serialVersionUID = -7423145010485856712L;
  
  public FuckOffException(String message){
    super(message);
  }
}
...
class MinHeap{
  ...
  public int deleteMinSiftUp(){
    //sift up basically works by replacing the deleted element with its smaller child and then recursing on the smaller child
    if(isEmpty()) throw new FuckOffException("Heap is empty");
    int retVal = data[0];
    int insAt = 1;
  ...
  public int deleteMinSiftDown(){
    //sift down basically works by replacing the deleted element with the largest element in the heap at this point
